...
from routes.shift_tracking import router as shift_tracking_router
...
shift_scheduler = AsyncIOScheduler(timezone="UTC")


async def backfill_tracking_tokens(db):
    """Give any pre-existing schedules a tracking token + empty tracking state
    so their shareable officer links work."""
    n = 0
    cursor = db.dispatch_schedules.find({"tracking_token": {"$exists": False}})
    async for doc in cursor:
        await db.dispatch_schedules.update_one({"_id": doc["_id"]}, {"$set": {
            "tracking_token": _uuid.uuid4().hex,
            "clock_in_at": doc.get("clock_in_at"),
            "clock_out_at": doc.get("clock_out_at"),
            "check_ins": doc.get("check_ins") or [],
            "last_check_in_at": doc.get("last_check_in_at"),
            "next_check_in_due_at": doc.get("next_check_in_due_at"),
            "emergency_clock_out": doc.get("emergency_clock_out"),
            "cancelled_at": doc.get("cancelled_at"),
            "geofence_exits": doc.get("geofence_exits") or [],
            "tracking_alerts": doc.get("tracking_alerts") or {"missed_checkin_windows": [], "missed_clockout_sent": False},
        }})
        n += 1
    if n:
        logger.info(f"Backfilled tracking tokens on {n} schedule(s)")
...
async def startup():
    try:
        ...
        try:
            await db.dispatch_schedules.create_index("tracking_token", unique=True, sparse=True)
        except Exception as e:
            logger.warning(f"Could not create tracking_token index: {e}")
        logger.info("Database indexes created")
        
        await seed_admin()
        await backfill_tracking_tokens(db)
        ...
        # Periodic scan for missed check-ins / missed clock-outs (every minute)
        try:
            if not shift_scheduler.running:
                shift_scheduler.add_job(
                    run_shift_alert_scan, "interval", seconds=60,
                    args=[db], id="shift_alert_scan", replace_existing=True,
                    max_instances=1, coalesce=True,
                )
                shift_scheduler.start()
                logger.info("Shift alert scheduler started")
        except Exception as e:
            logger.warning(f"Could not start shift scheduler: {e}")
    except Exception as e:
...
async def shutdown():
    try:
        if shift_scheduler.running:
            shift_scheduler.shutdown(wait=False)
    except Exception:
        pass
    client.close()
...
api_router.include_router(shift_tracking_router)
