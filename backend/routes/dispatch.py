...
def _doc_out(doc: dict) -> dict:
    ...
    result = _serialize(d)
    # Surface the shareable officer shift-tracking link when a token exists.
    if isinstance(result, dict) and result.get("tracking_token"):
        base = os.environ.get("FRONTEND_URL", "").rstrip("/")
        result["tracking_url"] = f"{base}/shift/{result['tracking_token']}"
    return result
...
async def create_schedule(payload: ScheduleCreate, request: Request, db=Depends(get_db)):
    ...
    for sched_date in dates:
        ...
        # Shift-tracking (Security Officer clock-in/out) fields
        doc["tracking_token"] = uuid.uuid4().hex
        doc["clock_in_at"] = None
        doc["clock_out_at"] = None
        doc["check_ins"] = []
        doc["last_check_in_at"] = None
        doc["next_check_in_due_at"] = None
        doc["emergency_clock_out"] = None
        doc["cancelled_at"] = None
        doc["geofence_exits"] = []
        doc["tracking_alerts"] = {"missed_checkin_windows": [], "missed_clockout_sent": False}
        doc["created_by"] = str(user["_id"])
        doc["created_at"] = now
        doc["updated_by"] = str(user["_id"])
        doc["updated_at"] = now
    ...
    for raw_row in reader:
        ...
        try:
            ...
            doc = {
                ...
                "clock_in_at": None, "clock_out_at": None, "check_ins": [],
                "last_check_in_at": None, "next_check_in_due_at": None,
                "emergency_clock_out": None, "cancelled_at": None, "geofence_exits": [],
...
async def live_tracking(request: Request, db=Depends(get_db)):
    ...
    the Live Tracking map. Position priority: latest check-in ping → clock-in
    location → the post site's geofence centre."""
    user = await get_current_user(request, db)
    require_permission(user, "dispatch.schedule.view")
