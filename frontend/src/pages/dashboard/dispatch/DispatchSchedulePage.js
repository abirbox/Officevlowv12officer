...
const DispatchSchedulePage = ({ todayOnly = false }) => {
  ...
  // Unique shift-tracking link for an officer's shift. Prefer the server-built
  // absolute URL; fall back to composing it from the token + current origin.
  const trackingUrl = (r) => r.tracking_url || (r.tracking_token ? `${window.location.origin}/shift/${r.tracking_token}` : null);
  const copyTrackingLink = async (r) => {
    const url = trackingUrl(r);
    if (!url) { toast.error('No tracking link available for this shift'); return; }
    try {
      await navigator.clipboard.writeText(url);
      toast.success('Shift tracking link copied');
    } catch (e) {
      // Clipboard API blocked (insecure context / permissions) — fall back.
      window.prompt('Copy this shift tracking link:', url);
    }
  };
  const openTrackingLink = (r) => {
    const url = trackingUrl(r);
    if (!url) { toast.error('No tracking link available for this shift'); return; }
    window.open(url, '_blank', 'noopener');
  };

  const openConfirm = (row, preselectStatus = null) => {    setConfDialog(row);
    setConfForm({
                  ...
                  <td className={`${tdPad} whitespace-nowrap ${cellBorder} ${stickyLastTd}`}>
                    <div className="flex items-center justify-center gap-0.5 flex-nowrap">
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-8 w-8 p-0 text-[#0EA5E9] hover:text-[#0284C7] shrink-0"
                        title="Copy shift tracking link"
                        aria-label="Copy shift tracking link"
                        data-testid={`copy-tracking-${r.id}`}
                        onClick={() => copyTrackingLink(r)}
                      >
                        <Link2 className="h-4 w-4" />
                      </Button>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button size="sm" variant="ghost" className="h-8 w-8 p-0 shrink-0" data-testid={`row-menu-${r.id}`} aria-label="Row actions">
                            <MoreVertical className="h-4 w-4" />
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" data-testid={`row-menu-content-${r.id}`}>
                          <DropdownMenuItem onClick={() => openTrackingLink(r)} data-testid={`open-tracking-${r.id}`}>
                            <ExternalLink className="h-4 w-4 mr-2" /> Open tracking page
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => copyTrackingLink(r)} data-testid={`copy-tracking-menu-${r.id}`}>
                            <Link2 className="h-4 w-4 mr-2" /> Copy tracking link
                          </DropdownMenuItem>
